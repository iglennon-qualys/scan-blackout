...
def getscanjobs(api: QualysAPI.QualysAPI, debug=False):
    ...
    for scan in response.findall('.//SCAN'):
        scanlist[scan.find('.//REF').text] = scan.find('.//TITLE').text
...
if __name__ == '__main__':
    ...
    if config['DEFAULT']['SkipScans'] == "True" or config['DEFAULT']['SkipScans'] == "Yes":
        skipscans = True
        skipprefix = config['DEFAULT']['SkipScanPrefix']
    else:
        skipscans = False
        skipprefix = ''
